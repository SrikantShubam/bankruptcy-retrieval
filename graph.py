...
def _download_candidate(candidate: Dict[str, Any], deal_id: str) -> Tuple[bool, str]:
    download_url = candidate.get("download_url") or candidate.get("absolute_url")
...
def run_pipeline(deals: List[Dict[str, Any]], ground_truth: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    retriever = RetrieverAgent(max_calls_per_deal=4)
    ...
    preflight_ok, _ = _preflight_courtlistener(os.getenv("COURTLISTENER_API_TOKEN", "").strip())
    ...
    for deal in deals:
        ...
        try:
            ...
            candidates, api_calls = retriever.execute_plan(plan.variants)
            ...
            for candidate in candidates[:3]:
                ...
                decision = decider.decide(deal, verification)
